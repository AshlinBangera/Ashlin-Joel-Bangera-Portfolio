...
const Library: React.FC = () => {
  return (
    <section id="library" className="py-24 bg-dark relative overflow-hidden border-t border-white/5">
      {/* Background Accents */}
      <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-blue-900/10 rounded-full blur-[120px] pointer-events-none"></div>
      <div className="absolute bottom-0 left-0 w-[300px] h-[300px] bg-accent/5 rounded-full blur-[80px] pointer-events-none"></div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-6">
            <div>
                <div className="inline-flex items-center gap-2 mb-2">
                    ...
                    <span className="text-xs font-bold text-accent tracking-widest uppercase">Active Inputs</span>
                </div>
                <h2 className="text-4xl font-bold">Library & Listening</h2>
                <p className="text-subtext mt-2 max-w-xl">Curated knowledge streams. Current reads, podcasts, and research papers fueling my thinking.</p>
            </div>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
                    ...
                    <div className="flex-1 overflow-y-auto pr-1 relative z-10 custom-scrollbar">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            {mediaList.map((item, idx) => (
                                <div key={idx} className="flex flex-col gap-2 group cursor-pointer p-2 rounded-xl hover:bg-black/5 transition-colors border border-transparent hover:border-black/5">
            ...
            <div className="lg:col-span-2 flex flex-col justify-center overflow-hidden h-[500px]">
                <div className="mb-6 flex items-center gap-4">
                     <h3 className="font-mono text-sm text-subtext uppercase tracking-widest">My Reading List</h3>
                     <div className="h-px bg-white/10 flex-1"></div>
                </div>
                
                {/* Marquee Container */}
                <div className="w-full overflow-hidden relative mask-image-linear-gradient">
                     <div 
                     ...
                     >
                         ...
                         {[...books, ...books].map((book, idx) => (
                             ...
                             >
                                 <div className="aspect-[2/3] rounded-lg overflow-hidden shadow-2xl border border-white/10 bg-card relative">
                                     <img 
                                        src={book.cover} 
                                        alt={book.title} 
                                     ...
                                     <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col items-center justify-center text-center p-4">
                                          <ExternalLink className="w-6 h-6 text-accent mb-2" />
                                          <p className="text-xs font-bold text-white line-clamp-2">{book.title}</p>
                                     </div>
                                 </div>
                                 <div className="mt-4">
                                     <h4 className="text-white font-bold text-sm truncate group-hover:text-accent transition-colors">{book.title}</h4>
                                     <p className="text-subtext text-xs truncate">{book.author}</p>
                                 </div>
                             </div>
                         ))}
                     </div>
